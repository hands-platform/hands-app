...
export async function GET(_request: Request, context: RouteContext) {
  ...
  if (!isVietnamOverviewGeoapifyTile(z, x, y)) {
    return NextResponse.json({ error: 'TILE_OUTSIDE_VIETNAM_OVERVIEW' }, { status: 404 });
  }

  const apiKey = process.env.GEOAPIFY_API_KEY?.trim();
  if (!apiKey) {
    return NextResponse.json({ error: 'GEOAPIFY_API_KEY_MISSING' }, { status: 503 });
  }

  const tileUrl = new URL(
    `https://maps.geoapify.com/v1/tile/${VIETNAM_GEOAPIFY_TILE_VIEW.style}/${z}/${x}/${y}.png`,
  );
  ...
  if (!response.ok || !response.body) {
    return NextResponse.json({ error: 'GEOAPIFY_TILE_UNAVAILABLE' }, { status: response.status });
  }
