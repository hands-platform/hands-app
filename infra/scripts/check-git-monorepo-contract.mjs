...
if (gitDir !== '.git') {
  fail('history-owned-by-root-git-dir', `Expected root .git directory, got ${gitDir ?? '<missing>'}.`);
}

if (!existsSync(join(root, '.git'))) {
  fail('root-git-dir-exists', 'Expected existing .git directory at repository root.');
}
