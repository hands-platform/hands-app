...
const patchOperationalPolicyValue = async (accessToken, key, value, { restoration = false } = {}) => {
  ...
  const smokeSecret =
    env.API_SMOKE_AUDIT_SECRET?.trim() || (env.NODE_ENV === 'production' ? '' : jwtAccessSecretFromEnv(env));
  if (!smokeSecret) {
...
await runCouponSmokeLifecycle({
  activate: () =>
    postJson(`/admin/coupons/${coupon.id}/activate`, adminAuth.accessToken, {
      reason: 'API smoke checkout verification',
    }),
  pause: () =>
    postJson(`/admin/coupons/${coupon.id}/pause`, adminAuth.accessToken, {
  ...
  run: async () => {
    ...
    const providerOnboardingAfterBasicProfile = await getJson(
      '/provider/onboarding',
      providerAuth.accessToken,
    );
    if (
    ...
    await postJson(
      `/admin/partners/${providerAuth.user.providerProfile.id}/kyc/approve`,
      adminAuth.accessToken,
    );
    ...
    const onboardingBankAccount = await postJson(
      '/provider/onboarding/bank-accounts',
      providerAuth.accessToken,
      {
    ...
    if (
      ...
      throw new Error(
        `Provider onboarding review flow failed: ${JSON.stringify(approvedProviderOnboarding)}`,
      );
    }
    ...
    const updatedSmokeTaxRule = await patchJson(
      `/admin/tax-rules/${smokeTaxRule.id}`,
      adminAuth.accessToken,
      {
        ...taxPolicyDraftEvidence,
        scope: 'DEFAULT',
    ...
    const partnerAliasInitialServices = await getJson(
      '/partner/services/groups',
      backupProviderAuth.accessToken,
    );
    ...
    if (!noCoordinateBrowseProviders.some((item) => item.id === providerAuth.user.providerProfile.id)) {
      throw new Error(
        `Customer partner browse without GPS coordinates should use the default browse origin.`,
      );
    }
    if (
    ...
    if (
      !preferredPartnerDistanceGateError.includes(
        'Preferred partner must be within 50km of the booking address',
      )
    ) {
      throw new Error(
    ...
    try {
      ...
      const preferredAcceptedParticipant = preferredAcceptPolicyMatchedBooking.participants?.find(
        (participant) =>
          participant.providerProfileId === preferredAcceptProviderAuth.user.providerProfile.id,
      );
    ...
    try {
      ...
      const narrowRadiusOpenBookings = await getJson(
        '/provider/bookings/open',
        backupProviderAuth.accessToken,
      );
      const narrowRadiusOpenBooking = narrowRadiusOpenBookings.find(
        (item) => item.id === narrowRadiusBooking.id,
      );
    ...
    try {
      ...
      const hasFcmDelivery = fcmPolicyNotification?.deliveries?.some(
        (delivery) => delivery.provider === 'FCM',
      );
      if (!hasFcmDelivery) {
        throw new Error(
    ...
    if (cancelledMomoBooking.status !== 'CANCELLED' || cancelledMomoBooking.payment?.status !== 'RELEASED') {
      throw new Error(
        `Cancelled booking did not release payment hold: ${JSON.stringify(cancelledMomoBooking)}`,
      );
    }
    if (
    ...
    const cancelledPaymentBeforeSync = await getJson('/admin/payments', adminAuth.accessToken).then(
    ...
    if (!noShowCustomerCancelError.includes('Booking cannot be cancelled in its current state')) {
      throw new Error(
        `No-show booking direct cancel returned an unexpected error: ${noShowCustomerCancelError}`,
      );
    }
    ...
    const walletDebtProviderNotifications = await getJson(
      '/notifications',
      walletDebtProviderAuth.accessToken,
    );
    if (
    ...
    if (
      ...
      throw new Error(
        `Cash debt split booking ledgers were not recorded: ${JSON.stringify(cashDebtEarning)}`,
      );
    }
    ...
    const matched = await postJson(
      `/customer/bookings/${booking.id}/select-provider`,
      customerAuth.accessToken,
      {
        providerId: providerAuth.user.providerProfile.id,
      },
    ...
    const publicProviderDetailAfterRepublish = await request(
      `/customer/partners/${reviewedProviderProfileId}`,
    );
    if (
    ...
    const completedCloseout = await postJson(
      `/admin/bookings/${booking.id}/closeout`,
      adminAuth.accessToken,
      {
    ...
    const completedSettlementSnapshot = (
      await getJson(
        '/admin/booking-settlement-snapshots?range=all&review=posted&take=100',
        adminAuth.accessToken,
      )
    ...
    if (
      !manualWalletSameAdminFailure.includes(
        'Manual wallet adjustment requires approval from a different admin',
      )
    ) {
      throw new Error(
    ...
    const manualWalletAdjustmentJournalSummary = (
      await getJson(
        '/admin/accounting-journal-batches?range=all&review=posted&take=100',
        adminAuth.accessToken,
      )
    ).find(
      (journal) =>
    ...
    if (
      companyBankAccountCreated.status !== 'INACTIVE' ||
      companyBankAccountCreated.metadata?.pendingApproval
    ) {
      throw new Error(
  ...
  financeApproverAuth.accessToken,
    ...
    if (
      ...
      customPricePayoutSnapshot.netCompanyFeeBeforeWithholding !==
    ...
    if (payoutBatchUpdate.transferRef !== `${payoutBatch.transferRef}-UPDATED`) {
      throw new Error(
        `Payout batch transfer reference was not updated: ${JSON.stringify(payoutBatchUpdate)}`,
      );
    }
    ...
    if (
      !payoutBatchSameAdminFailure.includes(
        'Payout batch paid closeout requires approval from a different admin',
      )
    ) {
      throw new Error(
    ...
    if (
      !adminBooking?.chatRoom?.id ||
      !adminBooking?.services?.length ||
      !adminBooking?.participants?.length
    ) {
    ...
    const adminHybridBookingDetail = await getJson(
      `/admin/bookings/${hybridBooking.id}`,
      adminAuth.accessToken,
    );
    ...
    if (
      adminCancelledBooking?.status !== 'CANCELLED' ||
      adminCancelledBooking?.payment?.status !== 'RELEASED'
    ) {
      throw new Error(
    ...
    if (
      adminNoShowBooking?.closedByRole !== 'ADMIN' ||
      adminNoShowBooking?.closedReason !== 'admin_no_show'
    ) {
      throw new Error(
    ...
    if (
      adminExpiredBooking?.closedByRole !== 'ADMIN' ||
      adminExpiredBooking?.closedReason !== 'admin_expired'
    ) {
      throw new Error(
    ...
    await postJson(
      `/admin/partners/${providerAuth.user.providerProfile.id}/ops-note`,
      adminAuth.accessToken,
      {
        note: partnerOpsNote,
    ...
    if (env.SHIFT_HANDOFF_LAUNCH_ENABLED === 'true') {
    ...
    if (refund) {
      ...
      const refundPaymentClearingEntry = (
        await getJson(
          '/admin/booking-payment-clearing?range=all&review=reversed&take=100',
          adminAuth.accessToken,
        )
      ).find(
        (entry) =>
      ...
      const refundedPaidEarning = adminEarningsAfterRefund.find(
        (earning) => earning.id === completedEarning.id,
      );
    ...
    const publicCatalogAfter = {
      ...
      optionCount: publicServiceGroupsAfter.reduce((total, group) => total + (group.options?.length ?? 0), 0),
    };
    if (
    ...
    console.log({
      ...
      providerPayoutHoldBlocked:
        Boolean(payoutHoldSanction.id) && liftedPayoutHoldSanction.status === 'LIFTED',
