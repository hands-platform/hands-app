...
export function parsePaymentsPerformanceArgs(argv) {
  ...
  return {
    ...
    review: values.review || 'history-released',
...
export function paymentsPerformanceTarget(databaseUrl, targetKind, allowLocal) {
  if (!databaseUrl) throw new Error('PAYMENTS_PERF_DATABASE_URL is required.');
  if (!['anonymized-staging', 'local', 'read-replica'].includes(targetKind)) {
    throw new Error('PAYMENTS_PERF_TARGET_KIND must be read-replica, anonymized-staging, or local.');
  }
  const parsed = new URL(databaseUrl);
  if (!['postgres:', 'postgresql:'].includes(parsed.protocol)) {
    throw new Error('PAYMENTS_PERF_DATABASE_URL must be a PostgreSQL URL.');
  }
  const isLocal = ['127.0.0.1', '::1', 'localhost'].includes(parsed.hostname);
  if (isLocal && (!allowLocal || targetKind !== 'local')) {
    throw new Error('Local measurement requires PAYMENTS_PERF_TARGET_KIND=local and --allow-local.');
  }
...
export function summarizePaymentsPlan(planDocument) {
...
async function main() {
  const args = parsePaymentsPerformanceArgs(process.argv.slice(2));
  const databaseUrl = process.env.PAYMENTS_PERF_DATABASE_URL?.trim();
  const targetKind = process.env.PAYMENTS_PERF_TARGET_KIND?.trim();
  const target = paymentsPerformanceTarget(databaseUrl, targetKind, args.allowLocal);
  ...
  try {
    ...
    const buildQueries = source.adminPaymentOperationsPerformanceQueries;
    if (typeof buildQueries !== 'function') {
      throw new Error('API dist is stale. Run the API build before this report.');
    }
    const queries = buildQueries(
      { range: 'all', review: args.review, sort: 'newest' },
      args.take,
      args.skip,
      new Date(),
    );
    ...
    const report = {
      ...
      queries: results,
      limitations: [
        'p95 is client-observed sequential read latency for this target, not HTTP latency.',
        'EXPLAIN ANALYZE executes each SELECT once before the p95 samples.',
        'No cache flush, ANALYZE, schema change, or index change is performed.',
      ],
    };
...
async function inspectDatabase(prisma, statementTimeoutMs) {
  return readOnlyTransaction(prisma, statementTimeoutMs, async (tx) => {
    ...
    return jsonSafe({
      ...
      pgStatStatements: extension.enabled,
      relations,
...
async function measureQuery(prisma, query, args) {
  return readOnlyTransaction(prisma, args.statementTimeoutMs, async (tx) => {
    ...
    return {
      ...
      plan: summarizePaymentsPlan(explainRows[0]?.['QUERY PLAN']),
    };
