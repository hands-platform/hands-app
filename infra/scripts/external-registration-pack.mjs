...
const registrationItems = [
  ...
  },
  {
    ...
    env: [
      ...
      envItem('SMS_PROVIDER', 'dev | twilio | viettel | fpt | custom', hasValue(env.SMS_PROVIDER)),
    ...
    setup: [
      ...
      'Use Twilio Verify for beta if convenient, then evaluate Viettel/FPT for Vietnam production delivery.',
  ...
  },
  {
    ...
    consolePath: 'Twilio Verify, Viettel, FPT, or selected SMS provider dashboard',
    env: [
      envItem('SMS_PROVIDER', 'twilio | viettel | fpt | custom', hasValue(env.SMS_PROVIDER)),
      envItem('SMS_API_URL', 'https://<sms-provider-api>', hasValue(env.SMS_API_URL)),
      envItem('SMS_API_KEY', '<sms-api-key>', hasValue(env.SMS_API_KEY)),
