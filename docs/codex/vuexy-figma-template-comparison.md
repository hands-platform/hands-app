...
| Figma design source, repo mirror | `C:/dev/massage-on-demand-vn/design/figma/vuexy-figma-dashboard-ui-kit-and-builder-v4.fig` | No | Intended stable repo reference | Not present by policy. The source package is about 167 MB and `design/figma/*.fig` is gitignored; see `design/figma/README.md`. Do not commit copied `.fig` files unless the team explicitly approves storing large design binaries in git. Keep using the ThemeForest source path as the readable Figma source. |
