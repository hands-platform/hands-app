...
| Repo-local Figma copy requested by design note | `C:/dev/massage-on-demand-vn/design/figma/vuexy-figma-dashboard-ui-kit-and-builder-v4.fig` | No | Optional local mirror only. | This path is currently missing. The authoritative readable source is the Vuexy package path above. Do not add the large binary to git unless explicitly approved. |
...
| Topbar | HANDS topbar, breadcrumbs, logout/session controls. | Navbar and layout components under `src/@layouts` and examples. | Navbar/header atoms. | Visual polish is partially aligned; auth/session behavior is HANDS-specific. | Continue using shared topbar atoms, icon-only logout, breadcrumbs, and protected-session flow. |
...
| Tables | `AdminDataTable` exists and is used increasingly. | MUI table examples, data grids, list pages. | Table atoms and data-display frames. | Some finance/detail tables still need consistent pagination/footer/action cells. | Keep moving raw table shells into `AdminDataTable`; preserve server pagination. |
...
| Forms | `AdminFormInput`, `AdminFormSelect`, `AdminFormDate`, `AdminFormTextarea`, buttons. | `CustomTextField`, MUI forms, ReactDatepicker wrapper. | Atoms for inputs, selects, textarea, checkbox, datepicker. | Some page-local CSS and edge states still need consolidation. | Update shared Atom CSS first, then page tests. |
...
| Inputs | Shared input atoms with Vuexy label model. | `CustomTextField` and form examples. | Input atoms. | Native date/time picker popup cannot be fully styled by CSS; wrapper is aligned. | Use ReactDatepicker for interactive calendar popovers; keep native fields only for compact submit forms. |
...
| Unauthorized page | Protected route/session flow exists. | Error/auth examples. | Error pages. | Needs periodic route guard checks. | Keep operator permission errors clear and linked to session. |
| 404 page | Route protection and not-found patterns exist. | Error examples. | Error pages. | Needs route/menu inventory guard for missed pages. | Prefer route inventory test before broad page work. |
