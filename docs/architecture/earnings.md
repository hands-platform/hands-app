...
For the MVP, the partner wallet guard reads the current `ProviderWalletLedgerEntry.amount` balance for the provider. `ProviderEarning.netAmount` remains the booking earning record and settlement source, but operational blocking must use the wallet ledger because manual deposits, manual adjustments, reversals, and payout retries can change the effective wallet balance without changing the original earning row.

- Positive delta: HANDS owes money to the partner.
- Negative delta: the partner owes HANDS fees/tax from cash bookings.

If the ledger wallet balance is negative, the API still allows marketplace list visibility and join records, but blocks final acceptance, service start, and payout release until the fee debt is cleared. It returns two messages:

- `walletBlockReason` / API `message`: `Outstanding HANDS fee settlement must be completed before final acceptance, service start, or payout release.`
- `walletBlockDisplayMessage` / API `displayMessage`: `Unpaid HANDS fees must be settled before final acceptance or service start.`
...
These ledger entries are the finance audit source for current wallet balance. Booking, payout, and admin operations may still inspect earning rows for evidence, but they must not use unsettled earning totals as the partner wallet blocking source.
