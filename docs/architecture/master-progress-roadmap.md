...
- `/operations-handoff` activity stream now sanitizes booking summaries, chat snippets, audit action labels, finance rows, partner records, and participant names before rendering. Full admin smoke confirms no visible legacy `Provider`, people-ranking, scoring, penalty, VIP, or gratuity wording across the guarded admin surface.
...
- Booking detail full record index now links directly to finance trace, earnings ledger, cash settlement desk, tax policy, and location trail so one booking can be used as the source-of-truth operations entry point.
...
- Booking monitor now presents the secondary participation lane as marketplace/candidate wording and keeps operator records factual, with no visible legacy secondary-participation or people-ranking language.
