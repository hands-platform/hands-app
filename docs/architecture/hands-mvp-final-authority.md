...
- Negative partner wallet balances allow visibility and join intent, but block final acceptance or final confirmation when the policy requires settlement.
...
- Negative wallet state is an operational settlement warning and final-acceptance gate, not a marketplace visibility gate.
